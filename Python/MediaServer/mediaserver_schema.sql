...
$BODY$
    WITH ins_storage AS (
        ...
        SELECT media_id FROM ins_storage
        )
        ,ins4 AS (
        INSERT INTO mediaserver.song
        SELECT media_id, title, songlength FROM ins_storage
        )
	,ins_song AS (
		INSERT INTO mediaserver.song_artists
		SELECT media_id, artistid FROM ins_storage
	)
        ,ins5 AS (
        INSERT INTO mediaserver.metadata (md_type_id,md_value)
        ...
        ,ins6 AS (
        INSERT INTO mediaserver.mediaitemmetadata
        SELECT media_id, genre_md_id FROM ins_storage, ins5
        )
        INSERT INTO mediaserver.mediaitemmetadata
        SELECT media_id, md_id FROM ins_storage, ins_md;
