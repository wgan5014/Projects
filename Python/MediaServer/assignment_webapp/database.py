...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        select *
        from
            mediaserver.useraccount join mediaserver.subscribed_podcasts sp using(username)
...
def user_in_progress_items(username):
    ...
    try:
        ...
        FROM
            mediaserver.mediaitem JOIN mediaserver.usermediaconsumption USING(media_id)
        WHERE
...
def get_alltvshows():
    ...
    try:
        ...
        #############################################################################
        #sql = """
        #SELECT DISTINCT tvshow_title, COUNT(mediaserver.tvshowepisode_id)
        #FROM mediaserver.TVShow, mediaserver.TVEpisode;
        #"""
        sql = """
        select
            tvshow_id,
            ...
            count(episode)
        from
            mediaserver.tvshow left outer join mediaserver.tvepisode using(tvshow_id)
...
def get_song(song_id):
    ...
    try:
        ...
        #sql = """
        #SELECT *
        #FROM mediaserver.Song, mediaserver.Song_Artists
        #WHERE song_id=%s;
        #"""
        sql = """
        select song_title, artist_name, length, storage_location, media_id
        from
            mediaserver.song
                join
            mediaserver.mediaitem on (song_id=media_id)
                natural join
            mediaserver.song_artists
                join
            mediaserver.artist on (performing_artist_id = artist_id)
        where song_id = %s;
        """
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        from
            mediaserver.song
                join
            mediaserver.mediaitemmetadata on (song_id = media_id)
                natural join
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        #sql = """
        #SELECT *
        #FROM mediaserver.Podcast, mediaserver.PodcastMetaData
        #WHERE podcast_id=%s;
        #"""
        sql = """
        SELECT
    ...
    try:
        ...
        from
            ...
            mediaserver.album_songs
                join
            mediaserver.artist on (performing_artist_id = artist.artist_id)
        where album_id = %s;
        """
...
def get_genre_name(genre_id):
    """
    Get the name of a genre, for use in query 10
    """
    conn = database_connect()
    if (conn is None):
        return None
    ...
    try:
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting Genre with Genre ID: "+genre_id, sys.exc_info()[0])
        cur.close()
        conn.close()
        raise
    cur.close()                     # Close the cursor
...
def get_genre_songs(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT DISTINCT s.song_id AS item_id, s.song_title AS item_title, 'Song' AS item_type, t.md_type_name AS genre_type
FROM mediaserver.MediaItemMetaData i INNER JOIN mediaserver.MetaData m
ON i.md_id = m.md_id
INNER JOIN mediaserver.Song s
ON s.song_id = i.media_id
INNER JOIN mediaserver.MetaDataType t
ON t.md_type_id = m.md_type_id
WHERE m.md_id = %s;
        """
...
def get_genre_podcasts(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
            SELECT DISTINCT p.podcast_id AS item_id, p.podcast_title AS item_title, 'Podcast' AS item_type
            FROM mediaserver.MediaItemMetaData i INNER JOIN mediaserver.MetaData d
            ON i.md_id = d.md_id
            INNER JOIN mediaserver.Podcast p
            ON i.media_id = p.podcast_id
            WHERE d.md_id = %s;
        """
...
def get_genre_movies_and_shows(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
            SELECT DISTINCT m.movie_id AS item_id, m.movie_title AS item_title, 'Movie' AS item_type
FROM mediaserver.MediaItemMetaData i INNER JOIN mediaserver.MetaData d
ON i.md_id = d.md_id
INNER JOIN mediaserver.Movie m
ON m.movie_id = i.media_id
WHERE d.md_id = %s
UNION
SELECT DISTINCT s.tvshow_id AS item_id, s.tvshow_title AS item_title, 'TV Show' AS item_type
FROM mediaserver.MediaItemMetaData i INNER JOIN mediaserver.MetaData d
ON i.md_id = d.md_id
INNER JOIN mediaserver.TVShowMetaData m
ON m.md_id = d.md_id
INNER JOIN mediaserver.TVShow s
ON m.tvshow_id = s.tvshow_id
WHERE d.md_id = %s
ORDER BY item_id;
        """
...
def get_tvshowep(tvshowep_id):
    ...
    try:
        ...
        from mediaserver.TVEpisode te left outer join
            ...
            on (te.media_id=temd.media_id) join mediaserver.mediaitem mi on (te.media_id = mi.media_id)
        where te.media_id = %s"""
...
def get_movie(movie_id):
    ...
    try:
        ...
        on (m.movie_id=mmd.media_id) join mediaserver.mediaitem mi on (mmd.media_id = mi.media_id)
        where m.movie_id=%s;"""
...
def find_matchingmovies(searchterm):
    ...
    try:
        ...
        #############################################################################
        sql = '''SELECT * FROM mediaserver.movie WHERE LOWER(movie_title) LIKE %s;'''
        arg = '%' + searchterm.lower() + '%'
        # sql = """
        # SELECT * FROM mediaserver.movie
        # WHERE LOWER(movie_title) LIKE LOWER(CONCAT('%', %s, '%'));
        # """

        r = dictfetchall(cur,sql,(arg,))
        print("return val is:")
        print(r)
        cur.close()                     # Close the cursor
        conn.close()                    # Close the connection to the db
...
#   Add a new Song
#####################################################
def add_song_to_db(location, song_desc, title, length, genre, artist_id):
    """
    Get all the matching Movies in your media server
    """
    #########
    ...
    try:
        ...
        """
        # add the song
        cur.execute(sql,(location, song_desc, title, length, genre, artist_id))
        conn.commit()                   # Commit the transaction
        r = cur.fetchone()
        print("return val is:")
        print(r)
...
def get_song_genres():
    ...
    try:
        ...
        r = [genre for genre in r if not is_int(genre['md_value'])]
...
def get_user_media_consumption(username, media_id):
    ...
    try:
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting media consumption:", sys.exc_info()[0])
        cur.close()
        conn.close()
        raise
    cur.close()                     # Close the cursor
...
def create_new_media_consumption(username, media_id, progress):
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        ...
        # Try executing the SQL and get from the database
        sql = """
        INSERT INTO mediaserver.UserMediaConsumption VALUES (%s, %s, %s, %s, %s)
        """

        print(sql)

        cur.execute(sql,(username,media_id,1,str(progress),str(date.today()),))
        conn.commit()                  # Close the connection to the db
    except:
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting media consumption:", sys.exc_info()[0])
        cur.close()
...
def media_consumption_play(username, media_id, progress):
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        ...
        SET play_count=play_count + 1, progress=%s, lastviewed=%s
        WHERE username=%s AND media_id=%s
        """

        print(sql)

        cur.execute(sql,(str(progress),str(date.today()),username,media_id))
        conn.commit()
        #cur.close()                     # Close the cursor
        #conn.close()                    # Close the connection to the db
    except:
...
def media_consumption_update_progress(username, media_id, progress):
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
